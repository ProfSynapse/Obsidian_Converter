...
export function generateMarkdown({
  ...
  // Start with frontmatter if metadata exists
  const parts = [];
  if (Object.keys(metadata).length > 0) {
    parts.push('---', Object.entries(metadata).map(([key, value]) => `${key}: ${value}`).join('\n'), '---', '');
  }
  ...
  // Add main content
  const processedContent = Array.isArray(content) ? content.map(item => formatContentItem(item)).join('\n\n') : escapeMarkdown(content);
...
function formatList(items, ordered = false) {
  return items.map((item, index) => `${ordered ? `${index + 1}.` : '-'} ${escapeMarkdown(item)}`).join('\n');
}
