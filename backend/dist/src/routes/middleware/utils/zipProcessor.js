...
export async function handleConversion(type, content, name, apiKey) {
  try {
    const conversionResult = await textConverterFactory.convertToMarkdown(type, content, name, apiKey);
...
function getCategory(type) {
  return Object.entries(CATEGORIES).find(([_, types]) => types.includes(type.toLowerCase()))?.[0] || 'other';
}
...
async function processUrlContent(item, folder) {
  ...
  if (item.images?.length) {
    const assetsFolder = siteFolder.folder('assets');
    console.log(`Adding ${item.images.length} image(s) to assets for "${siteName}"`);
    for (const image of item.images) {
      try {
        if (!image.data || !image.name) {
          console.warn(`Incomplete image data for "${image.name}" in "${item.name}". Skipping.`);
          continue;
        }
        ...
        const safeImageName = sanitizeFilename(path.basename(image.name));
        assetsFolder.file(safeImageName, buffer, {
          binary: true
        });
        console.log(`Added image: "${safeImageName}"`);
      } catch (imgError) {
        console.error(`Error processing image "${image.name}" for "${item.name}":`, imgError);
      }
    }
  }
}
...
async function processYoutubeContent(item, folder) {
  ...
  if (item.images?.length) {
    ...
    console.log(`Adding ${item.images.length} image(s) to assets for "${item.name}"`);
    for (const image of item.images) {
      if (image.data && image.name) {
        const safeImageName = sanitizeFilename(path.basename(image.name));
...
async function processAudioContent(item, folder) {
  ...
  if (item.originalContent) {
    ...
    audioFolder.file(safeAudioName, item.originalContent, {
      binary: true
    });
...
function generateSummary(items) {
  ...
  return ['# Conversion Summary', '', `Generated: ${new Date().toISOString()}`, '', '## Statistics', `- Total Items: ${items.length}`, `- Successful: ${successful.length}`, `- Failed: ${failed.length}`, '', '## Successful Conversions', ...successful.map(item => `- **${item.name}** (${item.type})${item.images?.length ? ` - ${item.images.length} image(s)` : ''}`), '', failed.length ? ['## Failed Conversions', ...failed.map(item => `- **${item.name}**: ${item.error}`)].join('\n') : ''].filter(Boolean).join('\n');
}

// Export functions for use in other modules
export { processFileContent, processUrlContent, processYoutubeContent, processParentUrlContent, processAudioContent, generateSummary };