...
const CONFIG = {
  crawler: {
    ...
    excludePatterns: [/\.(pdf|zip|doc|docx|xls|xlsx|ppt|pptx)$/i,
    // Document files
...
class WebsiteCrawler {
  ...
  async crawl(startUrl) {
    try {
      ...
      while ((queue.length > 0 || activeTasks.length > 0) && this.discoveredUrls.size < CONFIG.crawler.maxPages) {
...
class UrlProcessor {
  ...
  async processUrls(urls) {
    ...
    const tasks = Array.from(urls).map(url => limit(async () => {
      try {
  ...
  sanitizeFilename(input) {
    ...
    return input.toLowerCase().replace(/^\/+|\/+$/g, '').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 100) || 'index';
  }
  ...
  generateIndex(parentUrl, pages) {
    ...
    return [`---`, `title: "${hostname} Archive"`, `description: "Website archive of ${hostname}"`, `date: "${new Date().toISOString()}"`, `tags:`, `  - website-archive`, `  - ${hostname.replace(/\./g, '-')}`, `---`, '', `# ${hostname} Website Archive`, '', '## Site Information', `- **Source URL:** ${parentUrl}`, `- **Archived:** ${new Date().toISOString()}`, `- **Total Pages:** ${pages.length}`, `- **Successful:** ${successfulPages.length}`, `- **Failed:** ${failedPages.length}`, '', '## Successfully Converted Pages', '', ...successfulPages.map(page => {
      const name = page.name.replace(/\.md$/, '');
      return `- [[${name}]] - [Original](${page.url})`;
    }), '', failedPages.length ? ['## Failed Conversions', '', ...failedPages.map(page => `- ${page.url}: ${page.error}`), ''].join('\n') : '', '## Notes', '', '- All images are stored in the `assets/` folder', '- Internal links are preserved as wiki-links', '- Original URLs are preserved in page metadata', '- Generated with Obsidian Note Converter'].join('\n');
  }
}
...
export async function convertParentUrlToMarkdown(parentUrl) {
  ...
  try {
    ...
    const allImages = processedPages.filter(p => p.success).flatMap(p => p.images || []).filter(img => img && img.data && img.name).map(img => ({
    ...
    return {
      ...
      files: [{
        ...
        content: index,
        type: 'text'
      }, ...processedPages.filter(p => p.success).map(({
        name,
        content
      }) => ({
