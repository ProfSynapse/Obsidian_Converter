...
function extractTextFromXml(xml) {
  ...
  return matches.map(match => match.replace(/<a:t>|<\/a:t>/g, '')).join(' ').trim();
}
...
export async function convertPptxToMarkdown(input, originalName, apiKey) {
  try {
    ...
    const slideFiles = Object.keys(zip.files).filter(fileName => /^ppt\/slides\/slide\d+\.xml$/.test(fileName)).sort((a, b) => {
    ...
    let markdown = [`# ${presentationName}`, '', '---', 'type: presentation', `created: ${new Date().toISOString()}`, `original: ${originalName}`, '---', '', ''].join('\n');
    const images = [];
