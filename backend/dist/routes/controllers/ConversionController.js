...
export class ConversionController {
  ...
  handleBatchConversion = async (req, res, next) => {
    try {
      ...
      console.log(`Processing batch conversion of ${allItems.length} items:`, allItems.map(i => ({
        name: i.name,
        type: i.type
      })));
      ...
      next(new AppError(error.message || 'Failed to process batch conversion', error.statusCode || 500));
    }
  };
