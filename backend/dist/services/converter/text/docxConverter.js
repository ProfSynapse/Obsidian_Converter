...
export async function convertDocxToMarkdown(buffer, originalName) {
  try {
    ...
    const options = {
      convertImage: mammoth.images.imgElement(async image => {
        try {
          ...
          return {
            src: 'error-processing-image'
          };
        }
      }),
      styleMap: ["p[style-name='Section Title'] => h1", "p[style-name='Subsection Title'] => h2"]
    };
    ...
    const markdown = ['---', `title: ${baseName}`, `attachmentFolder: attachments/${baseName}`, 'created: ' + new Date().toISOString(), '---', '', result.value].join('\n');
