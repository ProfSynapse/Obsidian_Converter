...
async function getPopplerPath() {
  if (process.platform === 'win32') {
    ...
    const possiblePaths = ['C:\\Program Files\\poppler-24.08.0\\Library\\bin', 'C:\\Program Files\\poppler\\Library\\bin', 'C:\\Program Files\\poppler-23.11.0\\Library\\bin', 'C:\\Program Files (x86)\\poppler\\Library\\bin', 'C:\\poppler\\Library\\bin', process.env.POPPLER_PATH].filter(Boolean);
...
export async function convertPdfToMarkdown(input, originalName, apiKey) {
  ...
  try {
    ...
    // Create frontmatter
    const frontmatter = ['---', `title: ${baseName}`, `created: ${new Date().toISOString()}`, `source: ${originalName}`, `type: pdf`, `pages: ${pdfData.numpages}`, `image_count: ${images.length}`, '---', ''].join('\n');

    // Process text content
    let textContent = pdfData.text.replace(/\f/g, '\n\n---\n\n').replace(/(\r\n|\r|\n){3,}/g, '\n\n').replace(/[^\S\r\n]+/g, ' ').trim();
