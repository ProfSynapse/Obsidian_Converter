...
const excludedDirs = ['.git', 'node_modules', 'dist', 'build', 'coverage', 'logs', 'tmp', '.vscode', '.svelte-kit', outputDir // Exclude the logs folder
];
const excludedFiles = ['.DS_Store', 'Thumbs.db', 'package-lock.json', 'yarn.lock', 'pnpm-lock.yaml', '.env', '.gitignore', outputFileName, 'codeSummary.js', 'codeSummary.cjs', '*.config.js',
// Exclude all config files ending with .config.js
...
async function getSortedItems(dir) {
  ...
  const sortedDirs = filteredItems.filter(item => item.isDirectory()).sort((a, b) => a.name.localeCompare(b.name));
  const sortedFiles = filteredItems.filter(item => item.isFile()).sort((a, b) => a.name.localeCompare(b.name));
...
${Object.entries(fileData).map(([file, data]) => {
