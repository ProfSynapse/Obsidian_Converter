...
*Generated on 11/14/2024, 4:05:13 PM*
...
/**
 * Configuration object for URL converter
 */
const CONFIG = {
  puppeteer: {
    launch: {
  ...
  api: {
    endpoints: {
      convert: 'http://localhost:3000/api/v1/convert/url'
    }
  },
  validation: {
    maxRetries: 3,
...
/**
 * Custom error class for URL conversion errors
 */
...
class UrlUtils {
  /**
   * Validates and normalizes a URL
   * @param {string|Object} urlInput - URL to process
   * @returns {string} Normalized URL
   * @throws {UrlConversionError} If URL is invalid
   */
  static normalizeUrl(urlInput) {
    try {
      // Handle different input types
      const urlString = typeof urlInput === 'string' 
        ? urlInput 
        : urlInput?.url || urlInput?.href || '';
...
class UrlConverter {
   ...
   */
  static async convertToMarkdown(urlInput, options = {}) {
    const {
      apiKey,
      originalName,
      retry = CONFIG.validation.maxRetries
    } = options;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new UrlConversionError(
          errorData.message || 'URL conversion failed',
          'API_ERROR',
          errorData
        );
      }
      ...
      return {
        content: result.content,
        images: result.images || [],
        name: UrlUtils.sanitizeFilename(originalName || new URL(url).hostname),
        success: true
      };
      ...
      return {
        ...
        images: [],
        name: UrlUtils.sanitizeFilename(originalName || 'unknown'),
        success: false,
        error: error.message
      };
    }
   ...
   * @private
   */
  static async makeRequest(url, options, retries = CONFIG.validation.maxRetries) {
    try {
      const response = await fetch(url, {
        ...options,
        timeout: CONFIG.validation.timeoutMs
      });
      return response;
    } catch (error) {
      if (retries > 0 && this.shouldRetry(error)) {
        await new Promise(resolve => 
          setTimeout(resolve, CONFIG.validation.retryDelay)
        );
  ...
  /**
   * Formats error content for response
   * @private
   */
  static formatErrorContent(error, urlInput, originalName) {
    return [
      `# Conversion Error`,
      '',
...
export const convertUrlToMarkdown = UrlConverter.convertToMarkdown.bind(UrlConverter);
```
