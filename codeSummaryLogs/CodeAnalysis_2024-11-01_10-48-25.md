...
*Generated on 11/1/2024, 10:48:26 AM*
 ...
 * @param {Object} content - The converted content object
 * @param {string} siteName - Name for the ZIP file
 * @returns {Promise<Buffer>} ZIP file buffer
 */
async function createConversionZip(content, siteName) {
  const zip = new JSZip();

  // For parent URL conversions with multiple files
  if (content.files) {
    ...
    if (content.index) {
      zip.file('index.md', content.index);
    }
  } else {
    // Single file conversion
    zip.file(`${siteName}.md`, content.content);
...
/**
 * Multer storage and upload configuration with enhanced error handling
 */
const multerConfig = {
  ...
  fileFilter: (req, file, cb) => {
    try {
      ...
      // Store file type in request for API key validation
      req.fileType = ext;
      cb(null, true);
...
/**
 * Enhanced validation middleware with conditional API key checking
 */
const validators = {
  apiKey: (req, _res, next) => {
    // Get file type from either the request body, uploaded file, or URL type
    const fileType = req.fileType || 
                    req.body.fileType || 
                    (req.file && path.extname(req.file.originalname).slice(1));

    // If file type doesn't require API key, skip validation
...
/**
 * Enhanced conversion handlers with improved error handling and logging
 */
const conversionHandlers = {
  parentUrl: async (req, res, next) => {
    try {
      ...
      const siteName = sanitizeFilename(new URL(parentUrl).hostname);

      // Generate ZIP file
      const zipBuffer = await createConversionZip(result, siteName);

      // Send ZIP file
      res.set('Content-Type', 'application/zip');
      res.set('Content-Disposition', `attachment; filename=${siteName}.zip`);
      res.send(zipBuffer);
  ...
  parentUrl: async (req, res, next) => {
    try {
        ...
        console.log('Parent URL type:', typeof parentUrl);

        // Ensure we're passing just the URL string
        const urlString = typeof parentUrl === 'object' ? parentUrl.url : parentUrl;

        const result = await textConverterFactory.convertToMarkdown(
            'parentUrl',
            urlString, // Pass the URL string directly
...
/**
 * Route handlers with health check
 */
const routes = {
  health: (_req, res) => {
    res.json({
...
router.post('/file',
  validators.apiKey, // Now conditional based on file type
  validators.file,
  (req, res, next) => {
    upload(req, res, err => {
...
/**
 * Master configuration object for converter operations
 */
const CONFIG = {
  puppeteer: {
    launch: {
 ...
 * Removes invalid characters and ensures safe filename
 */
function sanitizeFilename(input) {
  ...
  return input
    ...
    .replace(/-+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, CONFIG.obsidian.maxFilenameLength)
    .trim() || 'untitled';
}

/**
 * Creates Obsidian vault directory structure
 * @param {string} siteName - Site being processed
 * @returns {Promise<Object>} Path information
 */
async function createVaultStructure(siteName) {
  const sanitizedSiteName = sanitizeFilename(siteName);
  const vaultPath = path.join(CONFIG.obsidian.baseDir, sanitizedSiteName);
  const assetsPath = path.join(vaultPath, CONFIG.obsidian.imageFolder);

  await fs.mkdir(CONFIG.obsidian.baseDir, { recursive: true });
...
function normalizeUrl(url) {
  try {
    ...
    if (!CONFIG.processing.validProtocols.includes(urlObj.protocol)) {
...
/**
 * Scrapes all child URLs from a parent URL
 * @returns {Promise<Set<string>>} Discovered URLs
 */
  ...
  try {
    ...
    while (pendingUrls.size > 0) {
      ...
      try {
        ...
        await page.goto(currentUrl, CONFIG.puppeteer.navigation);

        const newUrls = await page.evaluate((parentHost) => {
          return Array.from(document.querySelectorAll('a[href]'))
            .map(a => {
              try {
                const url = new URL(a.href);
                return url.hostname === parentHost ? url.href : null;
              } catch {
                return null;
              }
            })
        ...
        newUrls.forEach(url => {
          links.add(url);
          if (!processedUrls.has(url)) pendingUrls.add(url);
        });
...
/**
 * Processes URLs in chunks for memory management
 * @param {Array<string>} urls - URLs to process
 * @param {Object} paths - Directory paths for output
 */
async function processUrlsInChunks(urls, paths) {
  if (!paths || !paths.vault || !paths.assets) {
    throw new Error('Invalid paths object provided to processUrlsInChunks');
  }

  const results = [];
  const chunks = Array.from(
    { length: Math.ceil(urls.length / CONFIG.processing.concurrentLimit) }, 
    (_, i) => urls.slice(i * CONFIG.processing.concurrentLimit, (i + 1) * CONFIG.processing.concurrentLimit)
  );
  ...
  for (const [index, chunk] of chunks.entries()) {
    ...
    const chunkResults = await Promise.all(chunk.map(async url => {
      try {
        const urlObj = new URL(url);
        const pageName = sanitizeFilename(urlObj.pathname || urlObj.hostname);
        const result = await convertUrlToMarkdown(url, pageName);

        if (result.images?.length) {
          for (const image of result.images) {
            if (!image?.name || !image?.data) continue;
            
            const imagePath = path.join(paths.assets, image.name);
            await fs.writeFile(imagePath, Buffer.from(image.data, 'base64'));
            
            result.content = result.content.replace(
              new RegExp(image.path, 'g'),
              path.join('..', CONFIG.obsidian.imageFolder, image.name)
            );
          }
        }

        const filePath = path.join(paths.vault, `${pageName}.md`);
...
/**
 * Creates index file for Obsidian vault
 */
async function createIndexFile(vaultPath, parentUrl, results) {
  const successfulPages = results.filter(r => r.success);
  const failedPages = results.filter(r => !r.success);

  const content = [
    `# Website Archive: ${parentUrl}`,
    '',
    '## Successfully Converted Pages',
    '',
    ...successfulPages.map(page => 
      `- [[${page.pageName}]] (${page.imageCount} images)`
    ),
    '',
    failedPages.length ? [
      '## Failed Conversions',
    ...
    `- Total Images: ${successfulPages.reduce((sum, page) => sum + page.imageCount, 0)}`,
    '',
    `Generated: ${new Date().toISOString()}`
  ].join('\n');

  await fs.writeFile(
    path.join(vaultPath, CONFIG.obsidian.indexFileName),
    content
  );
}

/**
 * Main function to convert parent URL to Obsidian vault
 */
export async function convertParentUrlToMarkdown(parentUrl, originalName, apiKey) {
  try {
    ...
    console.log(`Starting conversion of ${normalizedParentUrl}`);

    // Create vault structure
    const siteName = new URL(normalizedParentUrl).hostname;
    const paths = await createVaultStructure(siteName);
    
    console.log('Created vault structure:', paths);
    ...
    if (childUrls.size === 0) {
      return {
        content: [
          `# No Content Found: ${normalizedParentUrl}`,
          '',
          '> No pages were found to convert at this URL.',
          '',
          '## Details',
    ...
    // Process URLs
    const results = await processUrlsInChunks(Array.from(childUrls), paths);

    // Create index
    await createIndexFile(paths.vault, normalizedParentUrl, results);
    ...
    console.log('Conversion completed successfully:', stats);
...
/**
 * Configuration for various aspects of the converter
 */
const CONFIG = {
  puppeteer: {
    launch: {
...
/**
 * Validates and normalizes a URL
 * @param {string|Object} urlInput - URL string or object containing URL
...
function normalizeUrl(urlInput) {
  try {
    // Handle URL objects
    const urlString = typeof urlInput === 'object' ? urlInput.url || urlInput.href : urlInput.toString();
 ...
 * @returns {CheerioStatic} Modified Cheerio instance
 */
 ...
 * @param {string} baseName - Base name for image paths
 * @returns {Object} Processed content and images array
 */
function processImages($, content, baseName) {
  const images = [];
    ...
    try {
      if (src.startsWith('data:')) {
        ...
        if (matches) {
          ...
          const imageData = matches[2];
          const imageName = `image-${index + 1}.${imageType}`;
          const imagePath = `attachments/${baseName}/${imageName}`;
        ...
        // Log external images
        console.warn(`External image found: ${src}`);
      }
    } catch (error) {
...
/**
 * Creates metadata markdown for the page
 * @param {CheerioStatic} $ - Cheerio instance
...
export async function convertUrlToMarkdown(urlInput, originalName) {
  ...
  try {
    ...
    // Navigate to page
    try {
      await page.goto(url, CONFIG.puppeteer.navigation);
    ...
    const { content: processedContent, images } = processImages($, mainContent, baseName);
    ...
    const metadata = createMetadata($, url);
    ...
    console.error(`Error converting ${urlInput}:`, error);
    
    // Return formatted error as markdown
    return {
      content: [
        `# Error Converting ${urlInput}`,
        '',
        '> Failed to convert this page to markdown.',
        '',
        '```',
        `Error: ${error.message}`,
        '```',
