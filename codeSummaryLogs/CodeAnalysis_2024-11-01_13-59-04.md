...
*Generated on 11/1/2024, 1:59:05 PM*
 ...
 * @param {string} type - The type of the item ('file', 'url', 'parenturl')
 * @param {string} [fileType] - The file extension if type is 'file'
 * @returns {string} The category ('data', 'multimedia', 'text', 'web', 'others')
 */
function determineCategory(type, fileType) {
  if (type === 'url' || type === 'parenturl') return 'web';
 ...
 * @param {Array<Object>} items - Array of converted items
 * @returns {Promise<Buffer>} ZIP file buffer
 */
async function createBatchZip(items) {
  const zip = new JSZip();
  ...
  for (const item of items) {
    try {
      ...
      if (!item.success) {
        ...
        const errorFolder = zip.folder('errors') || zip.folder('errors');
        errorFolder.file(
      ...
      const category = item.category || determineCategory(normalizedType, item.fileType);
 ...
 * @param {Object} item - File conversion result
 * @param {JSZip} folder - ZIP folder to add content to
 */
  ...
  if (item.images?.length) {
    ...
    for (const image of item.images) {
      if (image.data && image.name) {
        assetsFolder.file(image.name, image.data, { base64: true });
      }
    }
  }
}
 ...
 * @param {Object} item - URL conversion result
 * @param {JSZip} folder - ZIP folder to add content to
 */
 ...
 * @param {Object} item - Parent URL conversion result
 * @param {JSZip} folder - ZIP folder to add content to
 */
async function processParentUrlContent(item, folder) {
  const siteName = sanitizeFilename(new URL(item.originalUrl).hostname);
 ...
 * @param {Array<Object>} items - Converted items
 * @returns {string} Summary markdown content
 */
function generateSummary(items) {
  ...
  return [
    ...
    ...successful.map(item =>
      `- **${item.name}** (${item.type})${item.images?.length ? ` - ${item.images.length} images` : ''}`
    ),
    '',
    failed.length ? [
      '## Failed Conversions',
...
const conversionHandlers = {
  file: async (req, res, next) => {
    try {
      ...
      const category = determineCategory('file', fileType);
      ...
      const zipBuffer = await createBatchZip([{
        success: true,
        type: 'file',
        category: category, // Set the category based on file type
      ...
      res.set({
        ...
        'Content-Disposition': `attachment; filename=${sanitizeFilename(req.file.originalname)}_converted.zip`
      });
      res.send(zipBuffer);
  ...
  url: async (req, res, next) => {
    try {
      ...
      const zipBuffer = await createBatchZip([{
        success: true,
        type: 'url',
        category: 'web', // Assign to 'web' category
      ...
      const zipFilename = `${sanitizeFilename(new URL(url).hostname)}_page_${new Date().toISOString().replace(/[:.]/g, '-')}.zip`;
  ...
  parenturl: async (req, res, next) => {
    try {
      ...
      const urlString = typeof parenturl === 'object' ? parenturl.url || parenturl.parenturl : parenturl;
      const result = await textConverterFactory.convertToMarkdown(
        'parenturl',
        urlString,
      ...
      // Set proper headers for ZIP file download
      const zipFilename = `${sanitizeFilename(new URL(urlString).hostname)}_archive_${new Date().toISOString().replace(/[:.]/g, '-')}.zip`;
...
/**
 * Batch conversion handler - processes multiple items
 */
async function handleBatchConversion(req, res, next) {
  try {
    ...
    const results = await Promise.all(items.map(async (item) => {
      try {
        ...
        // Determine category based on type and fileType (if applicable)
        let category = 'others';
        if (normalizedType === 'file') {
          const fileType = item.fileType || path.extname(item.name).split('.').pop().toLowerCase();
          category = determineCategory('file', fileType);
        } else if (normalizedType === 'url' || normalizedType === 'parenturl') {
          category = 'web';
        }

        // Check if API key is required
        if (requiresApiKey(normalizedType) && !apiKey) {
          throw new Error('API key required for this conversion type');
        }
        ...
        return {
          success: true,
          type: normalizedType,
          category: category, // Set the category based on type and file type
          name: item.name,
          originalUrl: (normalizedType === 'url' || normalizedType === 'parenturl') ? item.content : undefined,
          ...result
        };
      } catch (error) {
        console.error(`Batch conversion error for item "${item.name}": ${error.message}`);
        return {
          success: false,
          type: item.type, // Keep original casing for error reporting
          name: item.name,
          error: error.message
        };
      }
...
class TextConverterFactory {
  ...
  constructor() {
    this.converters = {
      ...
      parenturl: convertParentUrlToMarkdown, // Registered the new converter
...
class ContentTypeHandler {
  ...
  static async handleImage(url, name) {
    try {
      ...
      const imageName = `${name || 'image'}-${uuidv4().slice(0, 8)}.${extension}`;
...
export async function convertUrlToMarkdown(urlInput, originalName) {
  ...
  try {
    ...
    if (contentType.isImage) {
      return await ContentTypeHandler.handleImage(fullUrl, originalName);
    }
    ...
    if (!contentType.isHtml) {
      return {
        ...
        files: [{
          name: `${sanitizeFilename(new URL(url).pathname || 'index')}.md`,
          content: metadata + markdown
        }],
        images
      };
    ...
    // Navigate to page
    const response = await page.goto(fullUrl, CONFIG.puppeteer.navigation);
    if (!response.ok()) {
    ...
    HtmlProcessor.cleanupContent($);
    const mainContent = HtmlProcessor.extractMainContent($);

    // Process images
    const { content: processedContent, images } = await HtmlProcessor.processImages(
      $,
      mainContent,
      originalName
    );
    ...
    const metadata = HtmlProcessor.createMetadata($, fullUrl);
